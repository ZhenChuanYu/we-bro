...
package com.node.downloadprovider;
...
class DownloadNotification {
	...
	private void updateActiveNotification(Collection<DownloadInfo> downloads) {
		...
		for (DownloadInfo download : downloads) {
			...
			item.mItemName = item.mId + item.mPackageName;
			item.addItem(title, progress, max);
			String tmp = "";
			android.util.Log.e("status", download.mStatus + "");

			item.mPaused = true;
			if (download.mStatus == Downloads.STATUS_PENDING) {
				tmp = "等待中...";
			} else if (download.mStatus == Downloads.STATUS_PAUSED_BY_APP) {
				tmp = "暂停中...";
			} else if (download.mStatus == Downloads.STATUS_QUEUED_FOR_WIFI
					|| download.mStatus == Downloads.STATUS_WAITING_FOR_NETWORK) {
				tmp = "等待网络中...";
			} else {
				item.mPaused = false;
			}
			item.mStatusText = tmp;
			mNotifications.put(item.mItemName, item);
		}
		...
		for (NotificationItem item : mNotifications.values()) {
			...
			int iconResource = item.mPaused ? android.R.drawable.stat_sys_warning
					: android.R.drawable.stat_sys_download;
			n.icon = iconResource;
			n.tickerText = item.mTitles[0] + "下载";
			n.flags |= Notification.FLAG_ONGOING_EVENT;

			// Build the RemoteView object
			RemoteViews expandedView = new RemoteViews(
					mContext.getPackageName(),
					R.layout.node_download_ongoing_notification);
			...
			expandedView.setTextViewText(R.id.status_text, item.mStatusText);
			...
			// TODO zhenchuan should overwrite n.contentIntent for nodebrowser
			...
			n.when = item.mId; // The notifications are ordered by this.
			android.util.Log.e("zhenchuan", item.mId + "");
